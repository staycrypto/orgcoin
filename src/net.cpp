...
static const int MAX_OUTBOUND_CONNECTIONS = 16;//16 connections max will allow us to communicate faster by reducing hops
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "OrgCoin " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    ...
    {"fallback.orgcoin.org","fallback.orgcoin.org"},
    {NULL, NULL}
};
...
// ^^^ These are IP addresses of your dns seeds in Hex format
unsigned int pnSeed[] =
{
    0x6BAAF49A,
    0x6BAA1AEF,
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. OrcCoin is probably already running."), addrBind.ToString().c_str());
        else
